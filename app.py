...
load_dotenv()
for _key in ("FIRECRAWL_API_KEY", "ANTHROPIC_API_KEY", "ODDS_API_KEY"):
    try:
...
# -- Session state -------------------------------------------------------------
for key in ["do_analysis", "f1", "f2", "total_stake", "cached_result", "pending_msg"]:
    if key not in st.session_state:
        if key == "do_analysis":
            st.session_state[key] = False
        elif key == "total_stake":
            st.session_state[key] = 100.0
        elif key == "cached_result":
            st.session_state[key] = None
        elif key == "pending_msg":
            st.session_state[key] = None
        else:
...
def render_cached_analysis(result: dict) -> None:
    ...
    f1_odds = odds_data["best_f1"]["odds"] if odds_data else None
    f2_odds = odds_data["best_f2"]["odds"] if odds_data else None

    st.divider()
    st.caption(f"Cached analysis · generated {result.get('generated_at', 'unknown')}")

    fighter_vs_header(f1_data["name"], f2_data["name"], f1_img, f2_img, f1_odds, f2_odds)

    if odds_data and odds_data.get("books"):
        annotated = annotate_odds_table(odds_data["books"], f1_data["name"], f2_data["name"])
        odds_comparison_table(_books_for_table(annotated))
    ...
    with col1:
        show_fighter_card(f1_data, image_url=f1_img, profile_text=analysis_sections.get("f1_profile"))
    with col2:
        show_fighter_card(f2_data, image_url=f2_img, profile_text=analysis_sections.get("f2_profile"))

    if analysis_sections.get("head2head"):
        analysis_section("Head-to-Head: Strengths & Weaknesses", analysis_sections["head2head"], icon="\U0001f94a")
    if analysis_sections.get("endings"):
        analysis_section("Most Likely Fight Endings", analysis_sections["endings"], icon="\U0001f3af")
...
if st.session_state.do_analysis:
    ...
    if not f1_name or not f2_name:
        st.error("Fighter names missing. Please go back and select a fight.")
        st.stop()
        ...
        if not os.environ.get("FIRECRAWL_API_KEY"):
            ...
            if card:
                ...
                if not card["fights"]:
                    ...
                    main_fights = [f for f in card["fights"] if f.get("section", "main") == "main"]
                    prelim_fights = [f for f in card["fights"] if f.get("section") == "prelim"]

                    clicked_main = render_card_section("Main Card", main_fights, "main")
                    clicked_prelim = render_card_section("Prelims", prelim_fights, "prelim")

                    chosen = None
                    if clicked_main is not None:
                        chosen = main_fights[clicked_main]
                    elif clicked_prelim is not None:
                        chosen = prelim_fights[clicked_prelim]

                    if chosen is not None:
                        cached = get_cached_analysis(chosen["fighter1"], chosen["fighter2"])
                        if cached is not None:
                            st.session_state.f1 = chosen["fighter1"]
                            st.session_state.f2 = chosen["fighter2"]
                            st.session_state.total_stake = 100.0
                            st.session_state.cached_result = cached
                            st.session_state.do_analysis = True
                            st.rerun()
                        else:
